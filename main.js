...
class EcovacsMap extends utils.Adapter {
  ...
  async onReady() {
    ...
    const geometrySeconds = this.clampNumber(
      this.config.geometryRefreshInterval,
      300,
      30,
      3600,
    );
    this.geometryTimer = this.setInterval(() => {
      this.refreshAllGeometry().catch((error) =>
        this.log.warn(`Geometry refresh failed: ${error.message}`),
      );
    }, geometrySeconds * 1000);

    const positionSeconds = this.clampNumber(
    ...
    this.positionTimer = this.setInterval(() => {
      this.refreshAllPositions().catch((error) =>
        this.log.warn(`Position refresh failed: ${error.message}`),
      );
  ...
  onUnload(callback) {
    try {
      if (this.geometryTimer) {
        this.clearInterval(this.geometryTimer);
      }
  ...
  async ensureQaObjects() {
    ...
    await this.ensureState("control.selfTest", {
      name: "Run ecovacs-map self test",
      type: "boolean",
      role: "button",
      read: false,
      write: true,
  ...
  async runSelfTest(trigger = "manual") {
    ...
    for (const device of this.devices.values()) {
      ...
      if (device.mapId) {
        add("OK", `${prefix}: Map-ID ${device.mapId}`);
      } else {
        add("WARN", `${prefix}: keine Map-ID erkannt.`);
      }

      if (device.roomIds.length) {
        add("OK", `${prefix}: ${device.roomIds.length} Raum/Räume erkannt.`);
      } else {
        add("WARN", `${prefix}: keine Räume erkannt.`);
      }

      if (device.transform) {
        add("OK", `${prefix}: Raumgeometrie/Transformation verfügbar.`);
      } else {
        add("WARN", `${prefix}: keine nutzbare Raumgeometrie/Transformation.`);
      }

      if (device.positionSource) {
        add("OK", `${prefix}: Positionsquelle ${device.positionSource}`);
      } else {
        add("WARN", `${prefix}: keine Live-Positionsquelle erkannt.`);
      }

      const svg = await this.getStateAsync(`${device.key}.map.svg`);
      if (svg && typeof svg.val === "string" && svg.val.includes("<svg")) {
        add("OK", `${prefix}: SVG-Ausgabe vorhanden.`);
      } else {
        add("WARN", `${prefix}: SVG-Ausgabe noch leer.`);
      }
    }
    ...
    this.log.debug(
      `Self test finished: ${status}, ${errors} error(s), ${warnings} warning(s).`,
    );
  ...
  detectDeviceName(prefix, states) {
    const candidates = [
      `${prefix}.info.deviceName`,
      `${prefix}.info.name`,
      `${prefix}.info.nickName`,
      `${prefix}.info.nickname`,
      `${prefix}.deviceName`,
      `${prefix}.name`,
    ];
  ...
  detectRoomIds(prefix, mapId, states) {
    ...
    const re = new RegExp(
      `^${escapedPrefix}\\.map\\.${escapedMap}\\.spotAreas\\.([^\\.]+)\\.`,
    );
    const out = new Set();
    for (const id of Object.keys(states)) {
      const match = id.match(re);
  ...
  detectRoomNameDetails(prefix, mapId, roomId, states) {
    ...
    for (const [suffix, score] of preferred) {
      add(`${base}.${suffix}`, score);
    }
    ...
    for (const id of Object.keys(states)) {
      ...
      if (!/(name|label|title|description)$/i.test(suffix)) {
        continue;
      }
      let score = 600;
      if (/custom|user/i.test(suffix)) {
        score += 300;
      }
      if (/room|area/i.test(suffix)) {
        score += 150;
      }
      add(id, score);
    }
    ...
    candidates.sort(
      (a, b) => b.score - a.score || b.name.length - a.name.length,
    );
  ...
  isGenericRoomName(name, roomId) {
    ...
    return (
      ...
      /^[A-Z]$/i.test(value) ||
      new RegExp(
        `^Raum\\s*${String(roomId).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`,
        "i",
      ).test(value)
    );
  }
  ...
  async resolveRoomName(device, roomId, states) {
    const detected = this.detectRoomNameDetails(
      device.prefix,
      device.mapId,
      roomId,
      states,
    ...
    for (const id of objectIds) {
      try {
        ...
        if (
          name &&
          !this.isGenericRoomName(name, roomId) &&
          !/^(name|label|room name|area name|spot area name)$/i.test(name)
        ) {
          return {
            name,
            id: `${id} [object.common.name]`,
            aliases: [detected.name],
          };
        }
      } catch {
  ...
  getUnifiedRoomConfigRows() {
    if (
      Array.isArray(this.config.roomConfig) &&
      this.config.roomConfig.length
    ) {
      return this.config.roomConfig;
    }
    ...
    const rows = [];
    for (const row of Array.isArray(this.config.roomNames)
      ? this.config.roomNames
      : []) {
      rows.push({
    ...
    for (const row of Array.isArray(this.config.roomMerges)
      ? this.config.roomMerges
      : []) {
      rows.push({
        deviceName: row?.deviceName || row?.device || "",
        sourceInstance:
          row?.sourceInstance ||
          (/^ecovacs-deebot\.\d+$/.test(String(row?.device || ""))
            ? row.device
            : ""),
  ...
  getRoomNameOverride(device, roomId) {
    ...
    const match = rows.find((row) => {
      const targetMatches =
        String(row?.sourceInstance || "") === String(device.prefix) ||
  ...
  getRoomMerges(device) {
    ...
    for (const row of rows) {
      const targets = [row?.sourceInstance, row?.deviceName, row?.device]
        .map((v) => String(v || "").trim())
        .filter(Boolean);
      if (
        !targets.some((target) =>
          [device.name, device.key, device.prefix].some(
      ...
      const ids = this.parseRoomMergeIds(row?.roomIds ?? row?.roomId).filter(
        (id) => device.roomIds.includes(id) && !used.has(id),
      );
      if (ids.length < 2) {
        continue;
      }
      const name =
        String(row?.customName || row?.mergedName || row?.name || "").trim() ||
        ids.map((id) => device.rooms.get(id)?.name || `Raum ${id}`).join(" + ");
  ...
  roomMergeForId(device, roomId) {
    return (
      this.getRoomMerges(device).find((group) =>
        group.ids.includes(String(roomId)),
      ) || null
    );
  }
  ...
  buildRoomMergeReport(device) {
    ...
    if (!groups.length) {
      return "Keine virtuelle Raum-Zusammenführung konfiguriert.";
    }
    return groups
  ...
  async syncRoomNameConfig() {
    try {
      ...
      if (!existing.length) {
        const migrated = [];
        for (const row of Array.isArray(native.roomNames)
          ? native.roomNames
          : []) {
          migrated.push({
            deviceName: row?.deviceName || "",
        ...
        for (const row of Array.isArray(native.roomMerges)
          ? native.roomMerges
          : []) {
          const target = String(
            row?.device || row?.deviceName || row?.sourceInstance || "",
          ).trim();
          const dev = [...this.devices.values()].find((d) =>
            [d.name, d.key, d.prefix].includes(target),
          );
          migrated.push({
            deviceName:
              dev?.name || (/^ecovacs-deebot\.\d+$/.test(target) ? "" : target),
            sourceInstance:
              dev?.prefix ||
              (/^ecovacs-deebot\.\d+$/.test(target) ? target : ""),
            roomIds: String(row?.roomIds || ""),
            ecovacsName: "",
            customName: row?.mergedName || row?.name || "",
          });
      ...
      for (const device of this.devices.values()) {
        ...
        const candidates = existing
          .filter((row) => {
            ...
            return targets.some((target) =>
              [device.name, device.key, device.prefix].includes(target),
            );
          })
          .sort(
            (a, b) =>
              this.parseRoomMergeIds(b?.roomIds ?? b?.roomId).length -
              this.parseRoomMergeIds(a?.roomIds ?? a?.roomId).length,
          );
        ...
        for (const row of candidates) {
          const ids = this.parseRoomMergeIds(
            row?.roomIds ?? row?.roomId,
          ...
          rows.push({
            ...
            customName: String(
              row?.customName || row?.mergedName || row?.name || "",
            ),
          });
        }

        for (const roomId of device.roomIds) {
          if (covered.has(String(roomId))) {
            continue;
          }
          ...
          rows.push({
            ...
            ecovacsName: String(
              room?.sourceName || room?.name || `Spot area ${roomId}`,
            ),
            customName: "",
          });
        }
      ...
      rows.sort(
        (a, b) =>
          ...
          Number(this.parseRoomMergeIds(a.roomIds)[0] || 0) -
            Number(this.parseRoomMergeIds(b.roomIds)[0] || 0),
      );

      if (
        JSON.stringify(native.roomConfig || []) === JSON.stringify(rows) &&
        native.roomNames === undefined &&
        native.roomMerges === undefined
      ) {
        return;
      }
      ...
      this.log.debug(
        `Updated unified room settings table with ${rows.length} row(s).`,
      );
    } catch (error) {
      this.log.debug(
        `Could not update unified room settings table: ${error.message}`,
      );
    }
  }
  ...
  async discover() {
    ...
    for (const old of this.devices.values()) {
      this.unsubscribeForeignStates(`${old.prefix}.*`);
    }
    this.devices.clear();
    ...
    for (const prefix of prefixes) {
      ...
      let key = this.sanitizeName(
        name,
        prefix.replace("ecovacs-deebot.", "Sauger_"),
      );
      if (this.devices.has(key)) {
        key = `${key}_${prefix.split(".").pop()}`;
      }
      ...
      const device = {
        ...
        finishCheckSeconds: null,
        customArea: null,
        customAreaSupported: false,
        customAreaRunActive: false,
        image: "",
        robotX: 0,
        robotY: 0,
        angle: 0,
      ...
      this.log.debug(
        `${name}: source=${prefix}, mapId=${mapId || "-"}, rooms=${roomIds.length}`,
      );
    }
    ...
    const legacyObjects = ["commands.rescan", "commands", "devices"];
    ...
    if (!this.devices.size) {
      this.log.warn("No ecovacs-deebot source instance/device detected.");
    }
  }
  ...
  async ensureDeviceObjects(device, states) {
    ...
    const savedRotation = await this.readOwnValue(
      [`${base}.map.rotation`, `${legacyBase}.map.rotation`],
      0,
    );
    const savedRobotSize = await this.readOwnValue(
      [`${base}.appearance.robotSize`, `${legacyBase}.map.robotSize`],
      4.5,
    );
    const savedHistoryMaxEntries = await this.readOwnValue(
      [`${base}.history.maxEntries`],
      100,
    );
    const savedHistoryEvents = await this.readOwnValue(
      [`${base}.history.events`],
      "",
    );
    const savedReportSequence = await this.readOwnValue(
      [`${base}.report.sequence`],
      0,
    );
    const savedCustomAreaActive = await this.readOwnValue(
      [`${base}.customArea.active`],
      false,
    );
    const savedCustomAreaValues = await this.readOwnValue(
      [`${base}.customArea.values`],
      "",
    );

    const customAreaObject = await this.getForeignObjectAsync(
      `${device.prefix}.control.customArea`,
    );
    ...
    const defs = [
      ...
      [
        "status.sourceAdapterVersion",
        {
          name: "ecovacs-deebot adapter version",
          type: "string",
          role: "text",
          read: true,
          write: false,
      ...
      [
        "status.geometryDetected",
        {
          name: "Room geometry detected",
          type: "boolean",
          role: "indicator",
          read: true,
          write: false,
      ...
      [
        "status.positionDetected",
        {
          name: "Live robot position detected",
          type: "boolean",
          role: "indicator",
          read: true,
          write: false,
      ...
      [
        "status.mapDetected",
        {
          name: "Map image source detected",
          type: "boolean",
          role: "indicator",
          read: true,
          write: false,
      ...
      [
        "status.controlsDetected",
        {
          name: "Cleaning controls detected",
          type: "boolean",
          role: "indicator",
          read: true,
          write: false,
      ...
      [
        ...
        {
          name: "Device compatibility diagnostic report",
          type: "string",
          role: "text",
          read: true,
          write: false,
      ...
      [
        "status.roomGeometryReport",
        {
          name: "Room geometry source diagnostic report",
          type: "string",
          role: "text",
          read: true,
          write: false,
      ...
      [
        "status.roomMergeReport",
        {
          name: "Configured virtual room merges",
          type: "string",
          role: "text",
          read: true,
          write: false,
      ...
      [
        ...
        {
          name: "Detected source state for robot position",
          type: "string",
          role: "text",
          read: true,
          write: false,
      ...
      [
        "status.rawPosition",
        {
          name: "Raw robot position from source adapter",
          type: "string",
          role: "text",
          read: true,
          write: false,
      ...
      [
        ...
        {
          name: "Robot position update status",
          type: "string",
          role: "text",
          read: true,
          write: false,
      ...
      [
        "status.selectionCaptureActive",
        {
          name: "App selection diagnostic capture active",
          type: "boolean",
          role: "indicator",
          read: true,
          write: false,
      ...
      [
        "status.selectionCaptureReport",
        {
          name: "Changed ecovacs-deebot states during app selection capture",
          type: "string",
          role: "text",
          read: true,
          write: false,
      ...
      [
        "control.refreshMap",
        {
          name: "Request fresh map image manually",
          type: "boolean",
          role: "button",
          read: false,
          write: true,
      ...
      [
        "control.captureAppSelection",
        {
          name: "Capture Ecovacs app room-selection changes for 30 seconds",
          type: "boolean",
          role: "button",
          read: false,
          write: true,
      ...
      [
        "control.lastSource",
        {
          name: "Last source command state",
          type: "string",
          role: "text",
          read: true,
          write: false,
      ...
      [
        "customArea.start",
        {
          name: "Start custom area cleaning",
          type: "boolean",
          role: "button",
          read: false,
          write: true,
      ...
      [
        "map.svg",
        {
          name: "SVG map without Ecovacs background",
          type: "string",
          role: "html",
          read: true,
          write: false,
      ...
      [
        "map.html",
        {
          name: "HTML map for VIS without Ecovacs background image",
          type: "string",
          role: "html",
          read: true,
          write: false,
      ...
      [
        "map.trail",
        {
          name: "Cleaning trail SVG points",
          type: "string",
          role: "text",
          read: true,
          write: false,
      ...
      [
        "map.coordinateMode",
        {
          name: "Coordinate transform mode",
          type: "string",
          role: "text",
          read: true,
          write: false,
      ...
      [
        "map.rotation",
        {
          ...
          write: true,
          def: 0,
          states: { 0: "0°", 90: "90°", 180: "180°", 270: "270°" },
        },
      ],
      [
        "map.bounds.minX",
      ...
      [
        "report.lastEventTime",
        {
          name: "Last report event time",
          type: "string",
          role: "date",
          read: true,
          write: false,
      ...
      [
        "report.sequence",
        {
          name: "Report event sequence number",
          type: "number",
          role: "value",
          read: true,
          write: false,
      ...
      [
        "history.count",
        {
          name: "Number of stored history events",
          type: "number",
          role: "value",
          read: true,
          write: false,
      ...
      [
        "history.maxEntries",
        {
          name: "Maximum number of history events",
          type: "number",
          role: "level",
          read: true,
          write: true,
      ...
      [
        "history.clear",
        {
          name: "Clear cleaning event history",
          type: "boolean",
          role: "button",
          read: false,
          write: true,
      ...
      [
        "appearance.robotSize",
        {
          name: "Robot marker radius",
          type: "number",
          role: "level",
          unit: "px",
          read: true,
    ...
    await this.setStateAsync(
      `${base}.status.roomsDetected`,
      device.roomIds.length,
      true,
    );

    device.rotation = this.normalizeRotation(savedRotation);
    device.robotSize = this.normalizeRobotSize(savedRobotSize);
    device.historyMaxEntries = this.normalizeHistoryMaxEntries(
      savedHistoryMaxEntries,
    );
    device.customAreaActive =
    ...
    const liveLines = device.historyEvents
      .slice(-5)
      .map((entry) =>
        entry.replace(/^\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s+/, ""),
      );

    device.reportCurrent = liveLines.join("\n");
    device.reportSequence = Number.isFinite(Number(savedReportSequence))
      ? Number(savedReportSequence)
      : 0;
    ...
    await this.setStateAsync(
      `${base}.appearance.robotSize`,
      device.robotSize,
      true,
    );
    await this.setStateAsync(
      `${base}.history.maxEntries`,
      device.historyMaxEntries,
      true,
    );
    await this.setStateAsync(
      `${base}.report.current`,
      device.reportCurrent,
      true,
    );
    await this.setStateAsync(
      `${base}.history.events`,
      device.historyEvents.join("\n"),
      true,
    );
    await this.setStateAsync(
      `${base}.history.count`,
      device.historyEvents.length,
      true,
    );
    await this.setStateAsync(
      `${base}.report.sequence`,
      device.reportSequence,
      true,
    );
    ...
    for (const obsolete of [
      ...
      try {
        ...
        if (obj) {
          await this.delObjectAsync(obsolete);
        }
      } catch (error) {
        this.log.debug(
          `Could not remove obsolete ${obsolete}: ${error.message}`,
        );
      }
    }
    ...
    for (const roomId of device.roomIds) {
      ...
      await this.ensureChannel(
        rb,
        this.detectRoomName(device.prefix, device.mapId, roomId, states),
      );
      await this.ensureState(`${rb}.name`, {
        name: "Room name",
        type: "string",
        role: "text",
        read: true,
      ...
      await this.ensureState(`${rb}.ecovacsName`, {
        name: "Original Ecovacs room name",
        type: "string",
        role: "text",
        read: true,
        write: false,
      ...
      await this.ensureState(`${rb}.label`, {
        name: "Short room label used on map",
        type: "string",
        role: "text",
        read: true,
        write: false,
      ...
      await this.ensureState(`${rb}.selected`, {
        name: "Selected for spot-area cleaning",
        type: "boolean",
        role: "switch",
        read: true,
        write: true,
      ...
      await this.ensureState(`${rb}.toggle`, {
        name: "Toggle room selection",
        type: "boolean",
        role: "button",
        read: false,
        write: true,
      ...
      await this.ensureState(`${rb}.selectionSource`, {
        name: "Source state(s) used for room selection sync",
        type: "string",
        role: "text",
        read: true,
        write: false,
      ...
      await this.ensureState(`${rb}.syncStatus`, {
        name: "Room selection synchronization status",
        type: "string",
        role: "text",
        read: true,
        write: false,
      ...
      await this.ensureState(`${rb}.clean`, {
        name: "Clean this room",
        type: "boolean",
        role: "button",
        read: false,
        write: true,
      ...
      await this.ensureState(`${rb}.polygon`, {
        name: "Scaled room polygon",
        type: "string",
        role: "text",
        read: true,
        write: false,
      ...
      await this.ensureState(`${rb}.centerX`, {
        name: "Room label X pixel",
        type: "number",
        role: "value",
        unit: "px",
        read: true,
      ...
      await this.ensureState(`${rb}.centerY`, {
        name: "Room label Y pixel",
        type: "number",
        role: "value",
        unit: "px",
        read: true,
      ...
      await this.ensureState(`${rb}.rawGeometry`, {
        name: "Raw room geometry",
        type: "string",
        role: "text",
        read: true,
        write: false,
      ...
      await this.ensureState(`${rb}.geometrySource`, {
        name: "Source state of room geometry",
        type: "string",
        role: "text",
        read: true,
        write: false,
      ...
      await this.ensureState(`${rb}.sourceState`, {
        name: "Source room channel",
        type: "string",
        role: "text",
        read: true,
        write: false,
      ...
      await this.ensureState(`${rb}.nameSource`, {
        name: "Source state/object of room name",
        type: "string",
        role: "text",
        read: true,
        write: false,
  ...
  normalizeImage(raw) {
    ...
    if (/^https?:\/\//i.test(value) || value.startsWith("/files/")) {
      return value;
    }
  ...
  findMapImage(device, states) {
    const preferred = device.mapId
      ? `${device.prefix}.map.${device.mapId}.map64`
      : "";
    if (preferred) {
      const image = this.normalizeImage(this.stateValue(states, preferred, ""));
      if (image) {
        return { id: preferred, image };
      }
    }
    const candidates = [];
    for (const [id, state] of Object.entries(states)) {
      if (!id.startsWith(`${device.prefix}.`)) {
        continue;
      }
      if (!/(map64|map.*image|image.*map|cleaning.*image)/i.test(id)) {
        continue;
      }
      ...
      if (/map.*image|image.*map/i.test(id)) {
        score += 500;
      }
      if (/cleaninglog/i.test(id)) {
        score -= 300;
      }
      candidates.push({ id, image, score });
    }
    candidates.sort(
      (a, b) => b.score - a.score || b.image.length - a.image.length,
    );
  ...
  geometryScore(id, nums) {
    ...
    if (/boundar(y|ies)/.test(n)) {
      score += 900;
    }
    if (/vertices|vertex/.test(n)) {
      score += 850;
    }
    if (/outline|contour/.test(n)) {
      score += 800;
    }
    if (/coordinates|coords/.test(n)) {
      score += 700;
    }
    if (/points/.test(n)) {
      score += 650;
    }
    if (/bounds|rectangle|rect/.test(n)) {
      score += 500;
    }
    if (/area/.test(n)) {
      score += 100;
    }
    if (/name|label|selected|markfornext|cleaning|id$/.test(n)) {
      score -= 1000;
    }
    if (/center|centroid|position/.test(n)) {
      score -= 700;
    }
    if (/customarea/.test(n)) {
      score -= 500;
    }
    if (nums.length >= 6) {
      score += Math.min(300, nums.length * 5);
    }
    if (nums.length === 4 && /bounds|rectangle|rect/.test(n)) {
      score += 200;
    }
    return score;
  }
  ...
  findRoomGeometry(device, roomId, states) {
    ...
    for (const [id, state] of Object.entries(states)) {
      ...
      if (score > 0) {
        candidates.push({ id, pairs, score });
      }
    }
    candidates.sort(
      (a, b) => b.score - a.score || b.pairs.length - a.pairs.length,
    );
  ...
  buildRoomGeometryReport(device, states) {
    ...
    for (const roomId of device.roomIds) {
      const roomBase = `${device.prefix}.map.${device.mapId}.spotAreas.${roomId}.`;
      const entries = Object.entries(states || {}).filter(([id]) =>
        id.startsWith(roomBase),
      );
      const usable = [];
      lines.push("");
      ...
      for (const [id, state] of entries) {
        ...
        if (pairs.length >= 3 && score > 0) {
          usable.push({ suffix, pairs: pairs.length, score });
        }
        ...
        const geometryHint =
          /(polygon|boundar|vertices|vertex|outline|contour|coordinates|coords|points|bounds|rectangle|rect)/i.test(
            suffix,
          );
        if (geometryHint || pairs.length >= 3) {
          lines.push(
            `  ${suffix}: nums=${nums.length}, pairs=${pairs.length}, score=${score}, Wert=${preview}`,
          );
        }
      }
      ...
      if (usable.length) {
        ...
        lines.push(
          `  verwendbar: ${usable.map((x) => `${x.suffix} (${x.pairs} Punkte, Score ${x.score})`).join(", ")}`,
        );
      } else {
        lines.push("  verwendbar: keine Geometrie erkannt");
        lines.push(
          `  vorhandene States: ${entries.map(([id]) => id.slice(roomBase.length)).join(", ")}`,
        );
      }
    }
    return lines.join("\n");
  ...
  isPixelLike(pairs) {
    return (
      pairs.length >= 3 &&
      pairs.every((p) => p.x >= -5 && p.x <= 545 && p.y >= -5 && p.y <= 236)
    );
  }

  createTransform(roomData, bounds) {
    const allPairs = roomData.flatMap((room) => room.pairs || []);
    if (!allPairs.length || !bounds) {
      return null;
    }
    const imgW = 540,
      imgH = 231,
      pad = 8;
    const pixelMode = allPairs.every(
      (p) => p.x >= -5 && p.x <= 545 && p.y >= -5 && p.y <= 236,
    );
    if (pixelMode) {
      return { mode: "pixel", imgW, imgH, pad };
    }
  ...
  scalePairs(device, pairs) {
    if (!pairs.length || !device.transform) {
      return [];
    }
  ...
  mapRoomLabel(name, roomId) {
    ...
    if (/^spot\s*area\s*\d+$/i.test(value) || /^raum\s*\d+$/i.test(value)) {
      return String(roomId);
    }
    return value;
  }
  ...
  async syncProjectedRuntime(device) {
    if (device.lastRawPosition) {
      const p = this.mapPoint(
        device,
        device.lastRawPosition.x,
        device.lastRawPosition.y,
      );
      device.robotX = p.x;
      device.robotY = p.y;
    }
    device.trail = (device.rawTrail || []).map((p) =>
      this.mapPoint(device, p.x, p.y),
    );
    const base = `${device.key}.map`;
    await this.setStateAsync(
      `${base}.robotX`,
      Number(device.robotX.toFixed(1)),
      true,
    );
    await this.setStateAsync(
      `${base}.robotY`,
      Number(device.robotY.toFixed(1)),
      true,
    );
    await this.setStateAsync(
      `${base}.trail`,
    ...
    await this.setStateAsync(
      `${base}.coordinateMode`,
      device.transform ? device.transform.mode : "",
      true,
    );
  }
  ...
  parsePosition(value) {
    ...
    if (
      ...
      return {
        x: Number(value.x),
        y: Number(value.y),
        angle: Number(value.angle || value.a || 0),
      };
    }
  ...
  async updateCompatibilityStatus(device, states = null) {
    ...
    const geometryDetected = [...device.rooms.values()].some(
      (room) => Array.isArray(room.rawPairs) && room.rawPairs.length >= 3,
    );
    const positionSource = this.detectPositionSource(device, states);
    const positionDetected = !!(
      states[positionSource] && this.parsePosition(states[positionSource].val)
    );
    ...
    for (const id of controlIds) {
      ...
      try {
        if (await this.getForeignObjectAsync(id)) {
          controlsDetected = true;
          break;
        }
      } catch {
    ...
    const issues = [];
    if (!device.mapId) {
      issues.push("keine Map-ID");
    }
    if (!device.roomIds.length) {
      issues.push("keine Räume");
    }
    if (!geometryDetected) {
      issues.push("keine Raumgeometrie");
    }
    if (!positionDetected) {
      issues.push("keine Live-Position");
    }
    if (!controlsDetected) {
      issues.push("keine Steuerstates");
    }
    if (issues.length) {
      compatibility =
        device.mapId && device.roomIds.length && geometryDetected
          ? "partial"
          : "limited";
    }
    ...
    await this.setStateAsync(
      `${base}.geometryDetected`,
      geometryDetected,
      true,
    );
    await this.setStateAsync(
      `${base}.positionDetected`,
      positionDetected,
      true,
    );
    await this.setStateAsync(`${base}.mapDetected`, mapDetected, true);
    await this.setStateAsync(
      `${base}.controlsDetected`,
      controlsDetected,
      true,
    );
  ...
  async refreshGeometry(device, states = null) {
    ...
    for (const roomId of device.roomIds) {
      ...
      const roomAliases = Array.isArray(roomNameInfo.aliases)
        ? roomNameInfo.aliases
        : [];
      ...
      await this.setStateAsync(
        `${rb}.sourceState`,
        `${device.prefix}.map.${device.mapId}.spotAreas.${roomId}`,
        true,
      );
      await this.setStateAsync(
        `${rb}.nameSource`,
        customRoomName ? "ecovacs-map settings" : roomNameInfo.id,
        true,
      );
      try {
        await this.extendObjectAsync(rb, { common: { name: roomName } });
      } catch {
        /* cosmetic only */
      }
      ...
      if (!found) {
        device.rooms.set(roomId, {
          ...
          points: "",
          source: "",
          selected: device.rooms.get(roomId)?.selected || false,
          center: { x: 0, y: 0 },
        });
        continue;
      }
      ...
      await this.setStateAsync(
        `${rb}.centerX`,
        Number(center.x.toFixed(1)),
        true,
      );
      await this.setStateAsync(
        `${rb}.centerY`,
        Number(center.y.toFixed(1)),
        true,
      );
    }
    await this.syncProjectedRuntime(device);
    await this.setStateAsync(
      `${device.key}.status.roomGeometryReport`,
      this.buildRoomGeometryReport(device, states),
      true,
    );
    await this.setStateAsync(
      `${device.key}.status.roomMergeReport`,
      this.buildRoomMergeReport(device),
      true,
    );
  }
  ...
  detectPositionSource(device, states) {
    ...
    if (states[exact] && this.parsePosition(states[exact].val)) {
      return exact;
    }
    ...
    for (const [id, state] of Object.entries(states || {})) {
      ...
      if (
        !/(deebot.*position|robot.*position|position.*deebot|position.*robot|map.*position)/i.test(
          id,
        )
      ) {
        continue;
      ...
      if (
        /spotarea|room|name|current.*area|charger|charge.*position/i.test(id)
      ) {
        continue;
      }
      if (!this.parsePosition(state && state.val)) {
        continue;
      }
      let score = 0;
      if (lower.endsWith(".map.deebotposition")) {
        score += 1000;
      }
      if (lower.includes("deebotposition")) {
        score += 700;
      }
  ...
  async refreshAllPositions() {
    ...
    for (const device of this.devices.values()) {
      ...
      await this.setStateAsync(
        `${device.key}.status.positionSource`,
        source,
        true,
      );
      ...
      if (!state || !this.parsePosition(state.val)) {
        await this.setStateAsync(
          `${device.key}.status.positionStatus`,
          `Keine gültige Position in ${source}`,
          true,
        );
        continue;
      }
      ...
      if (selectionChanged) {
        this.log.debug(
          `${device.name}: room selection mirrored from ecovacs-deebot/app`,
        );
      }
  ...
  async refreshRuntime(device, states = null) {
    ...
    await this.setStateAsync(
      `${device.key}.status.position`,
      String(this.stateValue(states, posNameId, "") || ""),
      true,
    );
    await this.setStateAsync(
      `${device.key}.status.positionSource`,
    ...
    await this.setStateAsync(
      `${device.key}.status.positionStatus`,
      this.parsePosition(rawPos)
        ? `Aktualisiert: ${posId}`
        : `Keine gültige Position in ${posId}`,
      true,
    );
    await this.refreshLiveReport(device, states);
  }
  ...
  async updateCleaningState(device, value) {
    ...
    const cleaningLogAvailable =
      !!states[cleanTypeId] && !!states[cleanedSecondsId];
    ...
    const chargingLike =
      s.includes("charging") || s.includes("charge") || s.includes("dock");
      ...
      device.finishCheckTimer = this.setTimeout(async () => {
        try {
          ...
          const latestSeconds = Number(
            this.stateValue(latestStates, cleanedSecondsId, 0) || 0,
          );
    ...
    if ((finished || logCanFinish) && device.wasCleaning) {
      await this.finishCleaningRun(device);
    }
  }
  ...
  classifyRobotStatus(value) {
    const s = String(value || "").toLowerCase();
    if (/return|returning|go.?home|homing|back.?to.?charge/.test(s)) {
      return "returning";
    }
    if (/washing/.test(s)) {
      return "washing";
    ...
    if (/charg|dock/.test(s)) {
      return "charging";
    }
    if (/pause/.test(s)) {
      return "paused";
    }
    if (/clean|spot|auto|edge/.test(s)) {
      return "cleaning";
    }
    if (/idle|standby|stop|finish|complete/.test(s)) {
      return "idle";
    }
    return s || "unknown";
  }
  ...
  currentTargetRoomNames(device, states) {
    ...
    for (const match of String(raw || "").matchAll(/-?\d+/g)) {
      ...
      if (!ids.includes(id) && device.rooms.has(id)) {
        ids.push(id);
      }
    }
    const names = ids.map((id) => {
      ...
      return (
        merge?.name ||
        device.rooms.get(id)?.name ||
        device.rooms.get(id)?.label ||
        `Raum ${id}`
      );
    });
  ...
  async appendHistoryEvent(device, text) {
    ...
    device.historyEvents = device.historyEvents.slice(
      -device.historyMaxEntries,
    );
    ...
    await this.setStateAsync(
      `${device.key}.report.current`,
      liveLines.join("\n"),
      true,
    );
    await this.setStateAsync(`${device.key}.report.lastEvent`, message, true);
    await this.setStateAsync(
      `${device.key}.report.lastEventTime`,
      now.toISOString(),
      true,
    );
    await this.setStateAsync(
      `${device.key}.report.sequence`,
      device.reportSequence,
      true,
    );
    await this.setStateAsync(
  ...
  async refreshLiveReport(device, states = null) {
    ...
    const roomRaw = this.stateValue(
      states,
      `${device.prefix}.map.deebotPositionCurrentSpotAreaName`,
      "",
    );
    ...
    const currentCleanType = String(
      this.stateValue(
        states,
        `${device.prefix}.cleaninglog.current.cleanType`,
        "",
      ) || "",
    ).trim();
    ...
    const currentCleanedSeconds = Number(
      this.stateValue(
        states,
        `${device.prefix}.cleaninglog.current.cleanedSeconds`,
        0,
      ) || 0,
    );
    ...
    const cleaningStationActive = this.isTrue(
      this.stateValue(
        states,
        `${device.prefix}.info.extended.cleaningStationActive`,
        false,
      ),
    );
    ...
    if (statusClass !== previousStatus) {
      if (statusClass === "cleaning") {
        if (previousStatus === "washing") {
          await this.appendHistoryEvent(device, "Reinigung wird fortgesetzt");
        } else {
        ...
        if (previousStatus === "returning" || previousStatus === "cleaning") {
          ...
          await this.appendHistoryEvent(device, "Lädt an der Ladestation");
        }
      } else if (statusClass === "paused") {
        if (cleaningStationActive && cleaningLogActive) {
          await this.appendHistoryEvent(device, "Wischen wird vorbereitet");
        } else {
        ...
        await this.appendHistoryEvent(device, "Wischpads werden getrocknet");
      } else if (
        statusClass === "idle" &&
      ...
      if (statusClass === "cleaning") {
        ...
        if (isTargetRoom) {
          await this.appendHistoryEvent(
            device,
            previousRoom
              ? `Raumwechsel – reinigt jetzt ${room}`
              : `Reinigt ${room}`,
          );
        } else {
  ...
  async resetRoomSelections(device) {
    for (const [roomId, room] of device.rooms.entries()) {
      ...
      try {
        ...
        if (object) {
          await this.setForeignStateAsync(source, false, false);
        }
      } catch (error) {
        this.log.debug(
          `${device.name}: could not reset ${source}: ${error.message || error}`,
        );
      }
    }
  }
  ...
  async updatePosition(device, value) {
    ...
    if (device.wasCleaning) {
      ...
      const lastPixel = lastRaw
        ? this.mapPoint(device, lastRaw.x, lastRaw.y)
        : null;

      if (
        !lastPixel ||
        Math.abs(lastPixel.x - pixel.x) > 1 ||
        Math.abs(lastPixel.y - pixel.y) > 1
      ) {
        device.rawTrail.push({ x: pos.x, y: pos.y });

        const limit = this.clampNumber(this.config.trailLimit, 500, 10, 5000);
        while (device.rawTrail.length > limit) {
          device.rawTrail.shift();
        }
      }
    ...
    await this.setStateAsync(
      `${base}.robotX`,
      Number(pixel.x.toFixed(1)),
      true,
    );
    await this.setStateAsync(
      `${base}.robotY`,
      Number(pixel.y.toFixed(1)),
      true,
    );
  ...
  detectAppSelectedRoomIds(device, states) {
    ...
    const cleanStatus = String(
      this.stateValue(states, `${device.prefix}.info.cleanstatus`, "") || "",
    ).toLowerCase();
    ...
    if (
      spotAreaRun &&
      currentUsedState &&
      currentUsedState.val !== null &&
      currentUsedState.val !== undefined
    ) {
      const raw =
        typeof currentUsedState.val === "string"
      ...
      for (const m of String(raw || "").matchAll(/-?\d+/g)) {
        ...
        if (roomIds.has(idValue)) {
          selected.add(idValue);
        }
      }
    ...
    for (const roomId of device.roomIds) {
      const roomBase = `${device.prefix}.map.${device.mapId}.spotAreas.${roomId}.`;
      for (const [id, state] of Object.entries(states)) {
        if (!id.startsWith(roomBase) || !state) {
          continue;
        }
        const tail = id.slice(roomBase.length);
        if (
          !/(mark.*clean|selected|selection|isSelected|activeFor.*clean)/i.test(
            tail,
          )
        ) {
          continue;
    ...
    if (device.wasCleaning && selected.size === 0) {
      const currentName = this.normalizeRoomText(
        this.stateValue(
          states,
          `${device.prefix}.map.deebotPositionCurrentSpotAreaName`,
          "",
  ...
  async refreshRoomSelections(device, states = null) {
    ...
    for (const roomId of device.roomIds) {
      ...
      const sourceSelected = sourceState
        ? this.isTrue(sourceState.val) &&
          sourceTs > 0 &&
          now - sourceTs <= 15000
        : false;
      const inferredSelected = appSelected.has(String(roomId));
      const currentUsedState =
        states[`${device.prefix}.map.currentUsedSpotAreas`];
      const cleanStatus = String(
        this.stateValue(states, `${device.prefix}.info.cleanstatus`, "") || "",
      ).toLowerCase();
      const currentUsedAuthoritative =
        /spot|area|room/.test(cleanStatus) && !!currentUsedState;
      ...
      if (manualBlock) {
        ...
        const genuinelyNewExplicitSelection =
          sourceSelected &&
          graceExpired &&
          sourceTs > Number(manualBlock.at || 0);
    ...
    const anyRoomSelected = [...device.rooms.values()].some(
  ...
  rotatedViewport(viewport, rotation) {
    ...
    if (r === 90 || r === 270) {
      return {
        ...
        width: h,
        height: w,
        value: `0 0 ${h.toFixed(1)} ${w.toFixed(1)}`,
        rotation: r,
      };
    }
    return {
      ...
      width: w,
      height: h,
      value: `0 0 ${w.toFixed(1)} ${h.toFixed(1)}`,
      rotation: r,
    };
  }
  ...
  rotationTransform(viewport, rotation) {
    ...
    if (r === 90) {
      return `translate(${h.toFixed(3)} 0) rotate(90) ${localize}`;
    }
    if (r === 180) {
      return `translate(${w.toFixed(3)} ${h.toFixed(3)}) rotate(180) ${localize}`;
    }
    if (r === 270) {
      return `translate(0 ${w.toFixed(3)}) rotate(270) ${localize}`;
    }
    return localize;
  }
  ...
  getSvgViewport(device) {
    ...
    for (const room of device.rooms.values()) {
      if (Array.isArray(room.scaledPairs)) {
        pts.push(...room.scaledPairs);
      }
    }
    if (Array.isArray(device.trail)) {
      pts.push(...device.trail);
    }
  ...
  lightenHex(color, amount = 0.42) {
    ...
    const out = parts.map((v) =>
      Math.max(0, Math.min(255, Math.round(v + (255 - v) * a))),
    );
  ...
  buildSvg(device, options = {}) {
    ...
    const addLabel = (name, center, click) => {
      ...
      const labelTransform = labelRotation
        ? ` transform="rotate(${labelRotation} ${center.x.toFixed(1)} ${center.y.toFixed(1)})"`
        : "";
      ...
      labels.push(
        `<text class="ecovacs-room-label" x="${center.x.toFixed(1)}" y="${center.y.toFixed(1)}"${labelTransform} text-anchor="middle" dominant-baseline="middle" vector-effect="non-scaling-stroke" style="${labelStyle}" onclick="${this.escapeXml(click)}">${this.escapeXml(name)}</text>`,
      );
    };
    ...
    for (const group of merges) {
      const members = group.ids
        .map((id) => device.rooms.get(id))
        .filter((room) => room?.points);
      if (!members.length) {
        continue;
      }
      ...
      const fillOpacity = selected ? 0.92 : 0.3;
      const click = clickScript(group.ids, selected);
      for (const room of members) {
        polygons.push(
          `<polygon points="${this.escapeXml(room.points)}" fill="${fillColor}" fill-opacity="${fillOpacity}" stroke="${fillColor}" stroke-width="0.35" vector-effect="non-scaling-stroke" style="cursor:pointer;pointer-events:auto" onclick="${this.escapeXml(click)}"><title>${this.escapeXml(group.name)}</title></polygon>`,
        );
      }
      ...
      const center = centers.length
        ? {
            x: centers.reduce((a, c) => a + c.x, 0) / centers.length,
            y: centers.reduce((a, c) => a + c.y, 0) / centers.length,
          }
        : null;
    ...
    for (const [roomId, room] of device.rooms.entries()) {
      if (!room.points || mergedIds.has(String(roomId))) {
        continue;
      }
      const color = colors[index++ % colors.length];
      ...
      polygons.push(
        `<polygon points="${this.escapeXml(room.points)}" fill="${fillColor}" fill-opacity="${fillOpacity}" stroke="${strokeColor}" stroke-width="${strokeWidth}" vector-effect="non-scaling-stroke" style="cursor:pointer;pointer-events:auto" onclick="${this.escapeXml(click)}"><title>${this.escapeXml(room.name)}</title></polygon>`,
      );
    ...
    if (device.customAreaActive && device.customArea) {
      ...
      if (values.length === 4 && values.every(Number.isFinite)) {
        ...
        const coordinateMode =
          device.transform?.mode === "pixel" ? "pixel" : "mapped";
    ...
    const trail = device.trail.length
      ? `<polyline points="${device.trail.map((p) => `${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(" ")}" fill="none" stroke="#ff3b30" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke" style="pointer-events:none"/>`
      : "";
  ...
  async rebuildView(device) {
    ...
    const viewport = device.svgViewport || {
      width: 540,
      height: 231,
      value: "0 0 540 231",
    };
  ...
  scheduleRebuild(device) {
    if (this.rebuildTimers.has(device.key)) {
      this.clearTimeout(this.rebuildTimers.get(device.key));
    }
    this.rebuildTimers.set(
      device.key,
      this.setTimeout(() => {
        this.rebuildTimers.delete(device.key);
        this.rebuildView(device).catch((error) =>
          this.log.warn(
            `${device.name}: view rebuild failed: ${error.message}`,
          ),
        );
      }, 150),
    );
  ...
  async findRoomSelectionSources(device, roomId) {
    ...
    for (const id of new Set([preferred, ...Object.keys(states)])) {
      ...
      try {
        ...
        if (object.common && object.common.write === false) {
          continue;
        }
        candidates.push(id);
      } catch {
  ...
  async syncRoomSelectionToSource(device, roomId, selected) {
    ...
    for (const source of sources) {
      try {
        ...
        this.log.debug(
          `${device.name} Raum ${roomId}: Auswahl konnte nicht an ${source} geschrieben werden: ${error.message}`,
        );
      }
    }
    ...
    await this.setStateAsync(
      `${ownBase}.syncStatus`,
      `An Ecovacs gesendet: ${action} (${sent}/${sources.length})`,
      true,
    );
    ...
    this.setTimeout(async () => {
      try {
        ...
        await this.setStateAsync(
          `${ownBase}.syncStatus`,
          confirmed
            ? `Ecovacs bestätigt: ${action}`
            : `Gesendet, aber nicht bestätigt: ${action}`,
          true,
        );
      } catch {
        /* ignore */
  ...
  async startSelectionCapture(device) {
    ...
    await this.setStateAsync(
      `${device.key}.status.selectionCaptureActive`,
      true,
      true,
    );
    await this.setStateAsync(
      `${device.key}.status.selectionCaptureReport`,
      "Aufzeichnung läuft 30 Sekunden – jetzt in der Ecovacs-App Raum/Räume auswählen.",
      true,
    );
    capture.timer = this.setTimeout(() => {
      this.finishSelectionCapture(device).catch((error) =>
        this.log.warn(
          `${device.name}: selection capture finish failed: ${error.message}`,
        ),
      );
    }, 30000);
  }
  ...
  async finishSelectionCapture(device) {
    ...
    if (capture.timer) {
      this.clearTimeout(capture.timer);
    }
    ...
    const report = rows.length
      ? rows
          .map(
            (row) =>
              `${row.id} = ${this.formatCaptureValue(row.val)} | ack=${row.ack} | Änderungen=${row.count}`,
          )
          .join("\n")
    ...
    this.log.debug(
      `${device.name}: App-Auswahl-Diagnose beendet, ${rows.length} geänderte States erkannt.`,
    );
  }

  recordSelectionCapture(device, id, state) {
    const capture = this.selectionCaptures.get(device.key);
    if (!capture || !state || !id.startsWith(`${device.prefix}.`)) {
      return;
    }
  ...
  async handleOwnState(id, state) {
    ...
    if (id === `${this.namespace}.${device.key}.customArea.active`) {
      ...
      if (active && !device.customAreaSupported) {
        device.customAreaActive = false;
        await this.setStateAsync(
          `${device.key}.customArea.active`,
          false,
          true,
        );
        this.log.warn(
          `${device.name}: custom area cleaning is not supported by the source adapter`,
        );
        this.scheduleRebuild(device);
        return;
      }
    ...
    if (id === `${this.namespace}.${device.key}.customArea.values`) {
      ...
      if (values.length !== 4 || !values.every(Number.isFinite)) {
        this.log.warn(
          `${device.name}: invalid custom area coordinates: ${state.val}`,
        );
        await this.setStateAsync(
          `${device.key}.customArea.values`,
          device.customArea || "",
    ...
    if (id === `${this.namespace}.${device.key}.customArea.start`) {
      ...
      if (!device.customAreaSupported || !device.customArea) {
        this.log.warn(
          `${device.name}: custom area start requested, but no supported custom area is available`,
        );
        await this.resetOwnButton(id);
        return;
      }
      ...
      if (values.length !== 4 || !values.every(Number.isFinite)) {
        this.log.warn(
          `${device.name}: invalid custom area coordinates: ${device.customArea}`,
        );
        await this.resetOwnButton(id);
        return;
      }
    ...
    if (id === `${this.namespace}.${device.key}.appearance.robotSize`) {
      ...
      await this.setStateAsync(
        `${device.key}.appearance.robotSize`,
        device.robotSize,
        true,
      );
      this.scheduleRebuild(device);
    ...
    if (
      id === `${this.namespace}.${device.key}.control.rotateLeft` ||
      id === `${this.namespace}.${device.key}.control.rotateRight`
    ) {
      if (!this.isTrue(state.val)) {
        return;
      }
    ...
    const roomMatch = id.match(
      new RegExp(
        `^${ns}\\.${key}\\.rooms\\.([^\\.]+)\\.(selected|toggle|clean)$`,
      ),
    );
    if (roomMatch) {
      ...
      const isSelected =
        action === "toggle"
          ? !(room?.selected === true)
          : this.isTrue(state.val);
      if (action === "toggle") {
        await this.resetOwnButton(id);
      }
      const localKey = `${device.key}:${roomId}`;
      const now = Date.now();
      this.localSelectionOverrides.set(localKey, {
        value: isSelected,
        until: now + 5000,
      });
      if (isSelected) {
      ...
      this.selectionGuards.set(`${device.key}:${roomId}`, {
        value: isSelected,
        until: Date.now() + 5000,
      });
      this.scheduleRebuild(device);
  ...
  async handleForeignState(id, state) {
    for (const device of this.devices.values()) {
      ...
      if (
        ...
        /(?:deebot|robot).*position|position.*(?:deebot|robot)/i.test(id)
      ) {
        await this.updatePosition(device, state.val);
        this.scheduleRebuild(device);
        return;
      ...
      if (id === `${device.prefix}.map.deebotPositionCurrentSpotAreaName`) {
        ...
        const states = await this.getAllSourceStates();
        const selectionChanged = await this.refreshRoomSelections(
          device,
          states,
        );
        await this.refreshLiveReport(device, states);
      ...
      const selected = id.match(
        new RegExp(
          `^${device.prefix.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\.map\\.${device.mapId.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\.spotAreas\\.([^\\.]+)\\.markForNextSpotAreaCleaning$`,
        ),
      );
      if (selected) {
        ...
        if (manualBlock) {
          ...
          const genuinelyNewExplicitSelection =
            isSelected &&
            now >= Number(manualBlock.until || 0) &&
            stateTs > Number(manualBlock.at || 0);
          if (!genuinelyNewExplicitSelection) {
            if (!isSelected) {
        ...
        if (guard && guard.value === isSelected) {
          this.selectionGuards.delete(guardKey);
        }
      ...
      if (
        id === `${device.prefix}.map.${device.mapId}.map64` ||
        /(map64|map.*image|image.*map)/i.test(id)
      ) {
        const states = await this.getAllSourceStates();
        await this.refreshMapImage(device, states);
        this.scheduleRebuild(device);
      ...
      if (
        id.startsWith(`${device.prefix}.map.${device.mapId}.spotAreas.`) &&
        /(polygon|boundar|vertices|vertex|outline|contour|coordinates|coords|points|bounds|rectangle|rect)/i.test(
          id,
        )
      ) {
