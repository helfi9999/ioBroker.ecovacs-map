...
class EcovacsMap extends utils.Adapter {
    ...
    isGenericRoomName(name, roomId) {
        ...
        return (
            !value ||
            /^[A-Z]$/i.test(value) ||
            new RegExp(`^Raum\\s*${String(roomId).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i').test(value)
        );
    }
    ...
    getUnifiedRoomConfigRows() {
        if (Array.isArray(this.config.roomConfig) && this.config.roomConfig.length) {
        ...
        for (const row of Array.isArray(this.config.roomMerges) ? this.config.roomMerges : []) {
            rows.push({
                ...
                sourceInstance:
                    row?.sourceInstance || (/^ecovacs-deebot\.\d+$/.test(String(row?.device || '')) ? row.device : ''),
    ...
    getRoomMerges(device) {
        ...
        for (const row of rows) {
            ...
            if (
                !targets.some(target =>
                    [device.name, device.key, device.prefix].some(value => String(value) === target),
                )
            ) {
                continue;
            }
        ...
        try {
            ...
            for (const device of this.devices.values()) {
                ...
                for (const row of candidates) {
                    const ids = this.parseRoomMergeIds(row?.roomIds ?? row?.roomId).filter(
                        id => device.roomIds.includes(id) && !covered.has(id),
                    );
                    if (!ids.length) {
                        continue;
                    }
    ...
    async ensureDeviceObjects(device, states) {
        ...
        device.customAreaSupported = customAreaObject?.type === 'state' && customAreaObject.common?.write === true;
    ...
    flattenNumbers(value) {
        ...
        const walk = v => {
            ...
            if (typeof v === 'object') {
                if (Object.prototype.hasOwnProperty.call(v, 'x') && Object.prototype.hasOwnProperty.call(v, 'y')) {
                    walk(v.x);
                    walk(v.y);
                    return;
                }
    ...
    async updateCompatibilityStatus(device, states = null) {
        ...
        if (issues.length) {
            compatibility = device.mapId && device.roomIds.length && geometryDetected ? 'partial' : 'limited';
        }
    ...
    detectPositionSource(device, states) {
        ...
        for (const [id, state] of Object.entries(states || {})) {
            ...
            if (!/(deebot.*position|robot.*position|position.*deebot|position.*robot|map.*position)/i.test(id)) {
                continue;
            }
            if (/spotarea|room|name|current.*area|charger|charge.*position/i.test(id)) {
                continue;
            }
    ...
    async refreshAllPositions() {
        ...
        for (const device of this.devices.values()) {
            ...
            await this.updateCompatibilityStatus(device, states);
            if (selectionChanged) {
                this.log.debug(`${device.name}: room selection mirrored from ecovacs-deebot/app`);
            }
    ...
    async refreshRuntime(device, states = null) {
        ...
        await this.setStateAsync(
            `${device.key}.status.positionStatus`,
            this.parsePosition(rawPos) ? `Aktualisiert: ${posId}` : `Keine gültige Position in ${posId}`,
            true,
        );
        await this.refreshLiveReport(device, states);
    }
    ...
    async updateCleaningState(device, value) {
        ...
        const cleaningLogAvailable = !!states[cleanTypeId] && !!states[cleanedSecondsId];

        const currentCleanType = String(this.stateValue(states, cleanTypeId, '') || '').trim();

        const currentCleanedSeconds = Number(this.stateValue(states, cleanedSecondsId, 0) || 0);

        const cleaningLogFinished = cleaningLogAvailable && currentCleanType === '' && currentCleanedSeconds === 0;
        const cleaning = s.includes('clean') || s.includes('spot') || s.includes('auto') || s.includes('edge');
        const finished = s.includes('idle') || s.includes('finish') || s.includes('complete') || s.includes('standby');

        const chargingLike = s.includes('charging') || s.includes('charge') || s.includes('dock');
            ...
            device.finishCheckTimer = this.setTimeout(async () => {
                try {
                    ...
                    const latestStatus = String(
                        this.stateValue(latestStates, `${device.prefix}.info.deviceStatus`, '') || '',
                    ).toLowerCase();

                    const latestSeconds = Number(this.stateValue(latestStates, cleanedSecondsId, 0) || 0);
                    ...
                    this.log.debug(`${device.name}: finish check failed: ${error.message || error}`);
                } finally {
                    device.finishCheckTimer = null;
                }
            }, 30000);
        ...
        const logCanFinish = cleaningLogFinished && ['charging', 'washing', 'drying'].includes(statusClass);
    ...
    displayRoomName(device, rawName) {
        ...
        for (const [roomId, room] of device.rooms.entries()) {
            const candidates = [room?.name, room?.sourceName, room?.label, ...(room?.aliases || []), roomId].map(
                value => this.normalizeRoomText(value),
            );
    ...
    async refreshLiveReport(device, states = null) {
            ...
            if (statusClass === 'cleaning') {
                if (previousStatus === 'washing') {
                    ...
                    await this.appendHistoryEvent(
                        device,
                        targets.length ? `Reinigung gestartet – fährt zu ${targets.join(', ')}` : 'Reinigung gestartet',
                    );
                }
            } else if (statusClass === 'returning') {
                ...
                if (previousStatus === 'returning' || previousStatus === 'cleaning') {
                    await this.appendHistoryEvent(device, 'An der Ladestation angekommen – lädt');
                } else {
        ...
        if (room && room !== previousRoom) {
            if (statusClass === 'cleaning') {
                ...
                await this.appendHistoryEvent(device, `Durchquert ${room} auf dem Weg zur Ladestation`);
            }
    ...
    detectAppSelectedRoomIds(device, states) {
        ...
        if (spotAreaRun && currentUsedState && currentUsedState.val !== null && currentUsedState.val !== undefined) {
            const raw =
                typeof currentUsedState.val === 'string' ? currentUsedState.val : JSON.stringify(currentUsedState.val);
        ...
        for (const roomId of device.roomIds) {
            ...
            for (const [id, state] of Object.entries(states)) {
                ...
                if (!/(mark.*clean|selected|selection|isSelected|activeFor.*clean)/i.test(tail)) {
                    continue;
                }
        ...
        if (device.wasCleaning && selected.size === 0) {
            const currentName = this.normalizeRoomText(
                this.stateValue(states, `${device.prefix}.map.deebotPositionCurrentSpotAreaName`, ''),
            );
            if (currentName) {
                for (const [roomId, room] of device.rooms.entries()) {
                    ...
                    if (
                        candidates.some(
                            name => currentName === name || currentName.includes(name) || name.includes(currentName),
                        )
                    ) {
                        selected.add(String(roomId));
                    }
    ...
    async refreshRoomSelections(device, states = null) {
        ...
        for (const roomId of device.roomIds) {
            ...
            if (manualBlock) {
                ...
                const genuinelyNewExplicitSelection =
                    sourceSelected && graceExpired && sourceTs > Number(manualBlock.at || 0);
        ...
        const anyRoomSelected = [...device.rooms.values()].some(room => room?.selected === true);
    ...
    buildSvg(device, options = {}) {
        ...
        if (device.customAreaActive && device.customArea) {
            const values = String(device.customArea).replace(/ /g, '').split(',').slice(0, 4).map(Number);
        ...
        this.rebuildTimers.set(
            ...
            this.setTimeout(() => {
                ...
                this.rebuildView(device).catch(error =>
                    this.log.warn(`${device.name}: view rebuild failed: ${error.message}`),
                );
            }, 150),
        );
    }
    ...
    async resetOwnButton(id) {
        try {
            await this.setStateAsync(id.slice(this.namespace.length + 1), false, true);
        } catch {
            /* ignore */
        }
    }
    ...
    async findRoomSelectionSources(device, roomId) {
        ...
        for (const id of new Set([preferred, ...Object.keys(states)])) {
            ...
            const tail = id.slice(roomBase.length);
            if (id !== preferred && !/(mark.*clean|selected|selection|isSelected|activeFor.*clean)/i.test(tail)) {
                continue;
            }
            try {
                const object = await this.getForeignObjectAsync(id);
    ...
    async startSelectionCapture(device) {
        ...
        capture.timer = this.setTimeout(() => {
            this.finishSelectionCapture(device).catch(error =>
                this.log.warn(`${device.name}: selection capture finish failed: ${error.message}`),
            );
        }, 30000);
    }
        ...
        for (const device of this.devices.values()) {
            ...
            if (selected) {
                ...
                if (manualBlock) {
                    ...
                    const genuinelyNewExplicitSelection =
                        isSelected && now >= Number(manualBlock.until || 0) && stateTs > Number(manualBlock.at || 0);
                    if (!genuinelyNewExplicitSelection) {
                        if (!isSelected) {
