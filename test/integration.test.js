        ...
        try {
          ...
          const rooms = await waitFor(async () => {
            const state = await getState(
              harness,
              "ecovacs-map.0.TestBot.status.roomsDetected",
            );
          ...
          const selected = await waitFor(async () => {
            const state = await getState(
              harness,
              "ecovacs-map.0.TestBot.rooms.2.selected",
            );
            return state && state.val === true ? state : null;
          });
          assert.equal(selected.val, true);

          const room0 = await getState(
            harness,
            "ecovacs-map.0.TestBot.rooms.0.selected",
          );
          const room1 = await getState(
            harness,
            "ecovacs-map.0.TestBot.rooms.1.selected",
          );
          ...
          const raw = await waitFor(async () => {
            const state = await getState(
              harness,
              "ecovacs-map.0.TestBot.status.rawPosition",
            );
          ...
          const angle = await waitFor(async () => {
            const state = await getState(
              harness,
              "ecovacs-map.0.TestBot.map.angle",
            );
          ...
          const svg = await waitFor(async () => {
            const state = await getState(
              harness,
              "ecovacs-map.0.TestBot.map.svg",
            );
            return state &&
              typeof state.val === "string" &&
              state.val.includes("<svg")
              ? state
              : null;
          });
          ...
          const liveReport = await waitFor(async () => {
            const state = await getState(
              harness,
              "ecovacs-map.0.TestBot.report.current",
            );
            return state && typeof state.val === "string" && state.val.trim()
              ? state
              : null;
          });
          ...
          const historyMax = await getState(
            harness,
            "ecovacs-map.0.TestBot.history.maxEntries",
          );
          ...
          const connection = await getState(
            harness,
            "ecovacs-map.0.info.connection",
          );
