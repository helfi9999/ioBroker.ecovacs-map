...
check(io.version === pkg.version, 'io-package root version matches package.json');
check(io.common && io.common.name === 'ecovacs-map', 'io-package common.name is ecovacs-map');
check(io.common && io.common.main === 'main.js', 'io-package main points to main.js');
check(fs.existsSync(path.join(root, io.common.main)), 'main.js exists');
...
check(fs.existsSync(path.join(root, 'CHANGELOG.md')), 'CHANGELOG exists');
